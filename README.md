...
Refer to [src/layers/python/layers/cognito_custom_challenge_helper.py](./src/layers/python/layers/cognito_custom_challenge_helper.py).

It is used to make it easy to handle custom challenge requests and responses.

### Define Auth Challenge

Refer to [src/define_auth_challenge/app.py](./src/define_auth_challenge/app.py).
...
Refer to [src/create_auth_challenge/app.py](./src/create_auth_challenge/app.py).
...
Environment variables `CODE_LENGTH` and `EMAIL_SENDER` are specified by an AWS SAM template described later.
...
Refer to [src/verify_auth_challenge/app.py](./src/verify_auth_challenge/app.py).
...
The `main.py` uses the following libraries to calculate values needed by [SRP - Secure Remote Password](https://en.wikipedia.org/wiki/Secure_Remote_Password_protocol).
